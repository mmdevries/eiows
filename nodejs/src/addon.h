...
void createGroup(const FunctionCallbackInfo<Value> &args) {
    uWS::Group<isServer> *group = hub.createGroup<isServer>(args[0].As<Number>()->Value(), args[1].As<Number>()->Value());
...
{
    uWS::OpCode opCode = (uWS::OpCode)args[2].As<Number>()->Value();
...
void closeSocket(const FunctionCallbackInfo<Value> &args) {
    ...
    unwrapSocket<isServer>(args[0].As<External>())->close(args[1].As<Number>()->Value(), nativeString.getData(), nativeString.getLength());
}
...
void closeGroup(const FunctionCallbackInfo<Value> &args) {
    NativeString nativeString(args.GetIsolate(), args[2]);
    uWS::Group<isServer> *group = (uWS::Group<isServer> *) args[0].As<External>()->Value();
    group->close(args[1].As<Number>()->Value(), nativeString.getData(), nativeString.getLength());
}
// TOT
template <bool isServer>
...
void prepareMessage(const FunctionCallbackInfo<Value> &args) {
    uWS::OpCode opCode = (uWS::OpCode)args[1].As<Number>()->Value();
...
void startAutoPing(const FunctionCallbackInfo<Value> &args) {
    ...
    group->startAutoPing(args[1].As<Number>()->Value(), std::string(nativeString.getData(), nativeString.getLength()));
}
